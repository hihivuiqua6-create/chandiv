...
# Đây là điểm tín hiệu nội bộ, không phải xác suất thắng.
# Giữ ngưỡng gốc; mặc định không chặn auto-bet vì người dùng đã chủ động bật.
MIN_CONFIDENCE_AUTO_BET = 60
AUTO_BET_REQUIRE_CONFIDENCE = False
AUTO_BET_RUN_UNTIL_STOP = True
# Gấp thếp làm tăng rủi ro thua vốn; mặc định tắt để tránh khuếch đại tổn thất.
...
# Trạng thái dùng chung có thể được chạm bởi polling thread, socket thread
# và watchdog cùng lúc. RLock + ghi file nguyên tử tránh mất dữ liệu/corrupt JSON.
STATE_LOCK = RLock()
SAVE_LOCK = RLock()
...
def save_data():
    """Ghi snapshot dữ liệu bằng file tạm rồi replace; an toàn khi nhiều thread cùng lưu."""
    try:
        with SAVE_LOCK:
            with STATE_LOCK:
                snapshot = {
...
def init_user_state(chat_id):
    ...
    if chat_id not in user_states:
        user_states[chat_id] = {
            ...
            'x2_on_lose': False,      # mặc định tắt gấp thếp; người dùng phải chủ động bật
            'always_bet': True        # bật auto thì giữ hành vi cược mỗi phiên như cấu trúc gốc
        }
    ...
    try:
        ...
        for p in lst:
            ...
            try:
                ...
                continue
            if res in ('TAI', 'XIU') and 3 <= len(dices) <= 3 and 3 <= tong <= 18:
                ketqua.append(res)
                diem.append(tong)
        return ketqua, diem
    except Exception as e:
...
def walk_forward_evaluate(history, points=None, min_history=12):
    """Đánh giá dự đoán theo thời gian, không nhìn trước kết quả tương lai.

    Trả về số mẫu, số đúng và accuracy thực nghiệm. Hàm chỉ dùng để kiểm thử,
    không biến accuracy quá khứ thành cam kết cho phiên tiếp theo.
    """
    history = [x for x in history if x in ('TAI', 'XIU')]
    points = list(points or [])
    if len(history) <= min_history:
        return {'samples': 0, 'correct': 0, 'accuracy': None}
    saved = {k: dict(v) for k, v in adaptive_model.items()}
    adaptive_model.clear()
    correct = 0
    samples = 0
    try:
        for i in range(min_history, len(history)):
            prefix = history[:i]
            pred = make_prediction_vip(prefix, points[:i])
            actual = history[i]
            samples += 1
            correct += int(pred == actual)
            for feature in extract_cau_features(prefix):
                row = adaptive_model.setdefault(feature, {'TAI': 0, 'XIU': 0})
                row[actual] += 1
    finally:
        adaptive_model.clear()
        adaptive_model.update(saved)
    return {'samples': samples, 'correct': correct,
            'accuracy': (correct / samples) if samples else None}


def tinh_do_tin_cay(history, points=None):
    """Điểm xếp hạng nội bộ, không phải xác suất thắng.

    Các mẫu cầu trong trò chơi may rủi không chứng minh được kết quả kế tiếp.
    Vì vậy điểm được chặn ở mức thận trọng và không hiển thị như cam kết thắng.
    """
    if len(history) < 5: return 50
    hs = "".join(['T' if x == 'TAI' else 'X' for x in history[-20:]])
    base = 52
    if re.search(r'TTTTTT$|XXXXXX$', hs): base = 66
    elif re.search(r'TTTTT$|XXXXX$', hs): base = 64
    elif re.search(r'TXTXTX$|XTXTXT$', hs): base = 63
    elif re.search(r'TTTT$|XXXX$', hs): base = 61
    elif re.search(r'TTXXTTXX$|XXTTXXTT$', hs): base = 60
    elif re.search(r'TTTXXX$|XXXTTT$', hs): base = 59
    elif re.search(r'TXTX$|XTXT$', hs): base = 57
    elif re.search(r'TTXX$|XXTT$', hs): base = 56
    elif re.search(r'TTT$|XXX$', hs): base = 55
    
    if len(history) >= 10:
        r10 = history[-10:]
        base += min(abs(r10.count('TAI') - r10.count('XIU')) * 2, 8)

    # Không nâng điểm chỉ vì một mẫu cầu thủ công; các mẫu này không
    # cung cấp xác suất thắng đã được kiểm định ngoài mẫu.
    # Nếu có dữ liệu học đủ lớn, dùng độ lệch thực nghiệm nhưng co về 50%.
    observed = []
    for feature in extract_cau_features(history):
        row = adaptive_model.get(feature, {})
        t, x = int(row.get('TAI', 0)), int(row.get('XIU', 0))
        n = t + x
        if n >= 20:
            observed.append(max(t, x) / n)
    if observed:
        empirical = sum(observed) / len(observed)
        base = max(base, int(50 + (empirical - 0.5) * 30))
    # Chỉ là điểm xếp hạng, không hiển thị như phần trăm thắng.
    return min(max(base, 50), 65)
...
║ 📊 ĐÃ THU THẬP: {n}/20 KẾT QUẢ</pre>"""
    ...
    def on_tick_update(data):
        ...
        if (gs == 'BETTING' and st['auto_bet_enabled'] and AUTO_BET_RUN_UNTIL_STOP
                and (not AUTO_BET_REQUIRE_CONFIDENCE or dt >= MIN_CONFIDENCE_AUTO_BET)):
            # Auto-bet bật thì gửi đúng một lệnh mỗi phiên; không gửi trùng nhờ betLock.
...
╚═══════════════════════════════╝</pre>"""
    ...
    def on_session_result(data):
        # Socket có thể phát lại event khi reconnect; xử lý trùng sẽ làm sai
        # thống kê, học online và gấp thếp.
        result_id = data.get('id') or data.get('sessionId') or data.get('session_id')
        if result_id is not None and result_id == st.get('last_processed_result_id'):
            logger['warn'](f"[{chat_id}] Bỏ qua session-result trùng: {result_id}")
            return
        if result_id is not None:
            st['last_processed_result_id'] = result_id

        st['betLock'] = False
        raw_dices = data.get('dices', [0, 0, 0])
        try:
            d = [int(x) for x in raw_dices]
        except (TypeError, ValueError):
            d = []
        if len(d) != 3 or any(x < 1 or x > 6 for x in d):
            logger['warn'](f"[{chat_id}] Dữ liệu xúc xắc không hợp lệ: {raw_dices!r}")
            return
        tong = sum(d)
