...
bot.set_my_commands([
    ...
    telebot.types.BotCommand("mox2khithua", "🔁 Bật gấp thếp X2 khi thua"),
    telebot.types.BotCommand("tatx2khithua", "🛡️ Tắt gấp thếp X2 khi thua"),
...
# ✅ AI TỰ HỌC THEO TỪNG LOẠI CẦU: {ten_cau: {'win': n, 'lose': n}}
pattern_stats = {}

def pattern_acc(ten):
    d = pattern_stats.get(ten)
    if not d: return 0.5, 0
    tot = d['win'] + d['lose']
    if tot == 0: return 0.5, 0
    return d['win'] / tot, tot

def update_pattern_stats(ten, ok):
    if not ten: return
    d = pattern_stats.setdefault(ten, {'win': 0, 'lose': 0})
    ...
    try:
        with open(SAVE_FILE, 'w', encoding='utf-8') as f:
            json.dump({'valid_keys': valid_keys, 'authorized_users': authorized_users,
                       ...
                       'pattern_stats': pattern_stats}, f, indent=2)
...
def nhan_dien_cau(history, points=None):
    ...
    def add(ten, du_doan, tin_cay, mo_ta):
        out.append({'ten': ten, 'du_doan': du_doan, 'tin_cay': int(tin_cay), 'mo_ta': mo_ta})
    ...
    if streak >= 4:
        ...
        if streak >= 8:
            # bệt quá dài → ưu tiên bẻ
            add(f"BẺ CẦU BỆT {streak}", _kq(_dao(last)), 86, f"Bệt {streak} nháy quá dài, xác suất gãy rất cao — ưu tiên bẻ")
        else:
            add(ten, _kq(last), min(72 + (streak - 4) * 6, 94), f"Bệt {streak} nháy liên tiếp → theo bệt")
    elif streak == 3:
        add("CẦU BỆT 3", _kq(last), 70, "Bệt 3 nháy → theo tiếp")

    # ─── 2. CẦU 1-1 (1 tài 1 xỉu liên tục) ───────────────────────
    if re.search(r'(TX){4}$|(XT){4}$', hs):
        add("CẦU 1-1 (8 nháy)", _kq(_dao(last)), 90, "1 Tài 1 Xỉu lặp 4 chu kỳ")
    elif re.search(r'(TX){3}$|(XT){3}$', hs):
        add("CẦU 1-1 (6 nháy)", _kq(_dao(last)), 87, "1 Tài 1 Xỉu lặp 3 chu kỳ")
    elif re.search(r'(TX){2}$|(XT){2}$', hs):
        add("CẦU 1-1 (4 nháy)", _kq(_dao(last)), 78, "1 Tài 1 Xỉu lặp 2 chu kỳ")

    # ─── 3. CẦU 2-2 / 3-3 / 4-4 / 5-5 ────────────────────────────
    for n, base in ((2, 86), (3, 84), (4, 82), (5, 83)):
        pat_a = ('T' * n + 'X' * n) * 2
        pat_b = ('X' * n + 'T' * n) * 2
        if hs.endswith(pat_a) or hs.endswith(pat_b):
            # đủ 2 chu kỳ → đang đúng nhịp
            tail = lens[-1]
            if tail < n:
                add(f"CẦU {n}-{n} (2 chu kỳ)", _kq(last), base, f"Lặp {n} Tài {n} Xỉu, đoạn hiện tại mới {tail}/{n}")
            else:
                add(f"CẦU {n}-{n} (2 chu kỳ)", _kq(_dao(last)), base, f"Lặp {n} Tài {n} Xỉu, đủ {n} → đảo chiều")
        elif hs.endswith('T' * n + 'X' * n) or hs.endswith('X' * n + 'T' * n):
            add(f"CẦU {n}-{n}", _kq(_dao(last)) if lens[-1] >= n else _kq(last),
                base - 8, f"Mẫu {n}-{n} vừa hình thành")

    # ─── 4. CẦU 3-2-1 và 1-2-3 ───────────────────────────────────
    if len(lens) >= 3:
        l3 = lens[-3:]
        if l3 == [3, 2, 1]:
            add("CẦU 3-2-1", _kq(_dao(last)), 84, "Chuỗi giảm dần 3→2→1, đảo chiều mỗi nháy")
        if l3 == [1, 2, 3]:
            add("CẦU 1-2-3", _kq(last), 82, "Chuỗi tăng dần 1→2→3, đoạn hiện tại còn kéo dài")
        if l3 == [2, 1, 2] and lens[-1] >= 2:
            add("CẦU 2-1-2", _kq(_dao(last)), 80, "Mẫu 2-1-2 hoàn tất → bẻ")
        if l3 == [1, 2, 1]:
            add("CẦU 1-2-1", _kq(last), 76, "Mẫu 1-2-1 → chuẩn bị đoạn 2")
    if len(lens) >= 4:
        l4 = lens[-4:]
        if l4 == [2, 1, 1, 2] and lens[-1] >= 2:
            add("CẦU 2-1-1-2", _kq(_dao(last)), 82, "Mẫu 2112 khép kín → đảo")
        if l4 == [1, 2, 1, 2] and lens[-1] >= 2:
            add("CẦU 1-2-1-2", _kq(_dao(last)), 81, "Mẫu 1212 → sang nhịp 1")
        if l4 == [3, 2, 1, 1]:
            add("CẦU 3-2-1-1", _kq(_dao(last)), 78, "Đuôi 3-2-1 chuyển sang nhịp 1-1")
        if l4 == [4, 3, 2, 1]:
    ...
    # ─── 5. CẦU 3 TÀI 3 XỈU (chuỗi 2 trở lên) ────────────────────
    if hs.endswith('TTTXXXTTTXXX') or hs.endswith('XXXTTTXXXTTT'):
        add("CẦU 3-3 CHUỖI DÀI", _kq(_dao(last)) if lens[-1] >= 3 else _kq(last), 88,
            "3 Tài 3 Xỉu lặp từ 2 chu kỳ trở lên")
...
def tong_hop_cau(history, points=None):
    ...
    st_, sx_ = 0.0, 0.0
    for c in ds:
        # ✅ AI TỰ HỌC: cầu nào lâu nay đánh đúng nhiều → cộng thêm trọng số
        acc, n = pattern_acc(c['ten'])
        hs_w = 1.0 + (acc - 0.5) * min(n, 20) / 20.0 * 1.2
        c['acc'] = int(acc * 100)
        c['lan'] = n
        w = c['tin_cay'] / 10.0 * hs_w
        if c['du_doan'] == 'TAI':
            st_ += w
        else:
            sx_ += w
    top = ds[0]
    if abs(st_ - sx_) < 0.5:
        return top['du_doan'], max(top['tin_cay'] - 12, 50), top, ds
    du_doan = 'TAI' if st_ > sx_ else 'XIU'
    tin_cay = top['tin_cay'] if top['du_doan'] == du_doan else max(top['tin_cay'] - 10, 55)
    # ✅ Điều chỉnh độ tin cậy theo lịch sử thắng/thua thực tế của cầu mạnh nhất
    acc, n = pattern_acc(top['ten'])
    if n >= 5:
        tin_cay = int(min(98, max(50, tin_cay + (acc - 0.5) * 24)))
...
def format_bang_cau(history, points=None, dices=None, limit=12):
    ...
    for c in ds[:6]:
        ...
        _extra = f" | 🎓 {c['acc']}% ({c['lan']} lần)" if c.get('lan') else ""
        lines.append(f"• {c['ten']} → {ic} ({c['tin_cay']}%){_extra}\n  <i>{c['mo_ta']}</i>")
...
def ai_tu_hoc(chat_id, du_doan, thuc_te):
    ...
    if du_doan == thuc_te:
        ...
        if not st.get('x2_khi_thua', True):
            # 🛡️ Người dùng đã /tatx2khithua → luôn giữ nguyên vốn gốc
            st['martingale_step'] = 0
            st['bet_amount'] = st['base_bet']
...
║ 📊 ĐÃ THU THẬP: {n}/20 KẾT QUẢ</pre>"""
        ...
        if n >= 3:
            ...
            if _top:
                ...
                msg += f"\n<pre>║ 🧠 CẦU: {_ten}</pre>"
                _acc, _n = pattern_acc(_top['ten'])
                if _n >= 3:
                    msg += f"\n<pre>║ 🎓 TỰ HỌC: cầu này đúng {int(_acc*100)}% ({_n} lần)</pre>"
                if _p and _c >= MIN_CONFIDENCE_AUTO_BET:
                    _ip = '🔵 TÀI' if _p == 'TAI' else '🔴 XỈU'
                    msg += f"\n<pre>║ 🤖 AI TỰ CHỐT THEO CẦU: {_ip} ({_c}%)</pre>"
...
║ 🔄 CƠ CHẾ X2: {'TẮT 🛡️' if not st.get('x2_khi_thua', True) else ('KÍCH HOẠT' if st['bet_amount'] > st['base_bet'] else 'GỐC')}
    ...
    def on_session_result(data):
        ...
        if kq in ['TAI', 'XIU']:
            ...
            if st['current_prediction']:
                # ✅ AI học theo loại cầu đã dùng để chốt phiên này
                update_pattern_stats(st.get('last_pattern'), st['current_prediction'] == kq)
...
║ 🎲 {d[0]}-{d[1]}-{d[2]} = {tong} → {icon}</pre>"""
        ...
        if st['current_prediction']:
            ...
            text_kq = f"🟢 ĐÚNG ✅ THẮNG LIÊN {st['win_streak']}" if ok else f"🔴 SAI ⚠️ THUA LIÊN {st['lose_streak']}" + (" (X2 PHIÊN SAU)" if st.get('x2_khi_thua', True) else " (GIỮ VỐN GỐC)")
            row += f"\n<pre>║ 📊 AI: {text_kq}</pre>"
...
def send_tat_x2(message):
    ...
    if not check_auth(cid):
        return bot.reply_to(message, locked_msg())
    st = user_states[cid]
    st['x2_khi_thua'] = False
    st['martingale_step'] = 0
    st['bet_amount'] = st['base_bet']
    bot.reply_to(message, f"🛡️ ĐÃ TẮT GẤP THẾP X2 KHI THUA\n💰 Mọi phiên đều đánh vốn gốc: {st['base_bet']:,} WIN\n▶️ Bật lại bằng /mox2khithua")
...
def send_mo_x2(message):
    ...
    st['bet_amount'] = st['base_bet']
    b = st['base_bet']
    bot.reply_to(message, f"🔁 ĐÃ BẬT GẤP THẾP X2 KHI THUA\n🔄 LỘ TRÌNH: {b:,} → {b*2:,} → {b*4:,} → {b*8:,}\n🛑 Thua ở {b*8:,} sẽ quay về vốn gốc.\n⏹️ Tắt bằng /tatx2khithua")
