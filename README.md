...
Sau khi bot online, dùng `/login TAIKHOAN MATKHAU`, sau đó dùng `/autobet on 2k` để đặt vốn gốc 2.000 WIN mỗi chu kỳ. Lộ trình gấp thếp được giới hạn là `2k → 4k → 8k → 16k`; nếu thua ở 16k, phiên kế tiếp quay lại 2k thay vì tăng lên 32k. Dùng `/autobet off` để tắt và reset về vốn gốc; dùng `/stop` để ngắt socket.
